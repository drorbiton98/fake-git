...
def diff_files_from_common_dirs(common_dirs, main_file, secondary_file):
    ...
    for subdir in common_dirs:
        ...
        left_only += compared.left_only
        right_only += compared.right_only
...
def changes_to_be_commited():
    ...
    return diff_files_from_common_dirs(common_dirs, live_file, last_file)[0] + files_from_diff_dirs(diff_dirs, live_file)
...
def changes_not_staged_for_commit():
    ...
    for path in original_paths:
        if os.path.isfile(path):
            ...
            files_after_comparison = diff_files_from_common_dirs(common_dirs, base_path, current_dir)[0]
...
def return_value(file1, file2):
    dir1 = {x[0].replace(file1, '') for x in os.walk(file1)}
    dir2 = {x[0].replace(file2, '') for x in os.walk(file2)}
    
    common_dirs = dir1.intersection(dir2)

    diff_files, left_only, right_only = diff_files_from_common_dirs(common_dirs, file1, file2)

    return_value = f"""
    in both files but different: {diff_files}
    only in {file1}: {left_only}
    only in {file2}: {right_only}
    """
    return return_value


def find_relative_path(path, head_path):
    dir_of_relative = collections.deque([])

    path, end = os.path.split(path)
    dir_of_relative.append(end)
    relative_path = os.path.join(head_path, *dir_of_relative)

    while not os.path.exists(relative_path):
        path, end = os.path.split(path)
        if end:
            dir_of_relative.appendleft(end)
            relative_path = os.path.join(head_path, *dir_of_relative)
        else:
            raise PathNotFoundError("sorry, couldn't find the given in path backup in the current working directory tree")
    
    return relative_path


def single_file(path, head=None):
    ...
    if head == '--cached':
        head_path = os.path.join(current_dir, '.wit', 'staging_area')
        head_id = 'staging_area'
    elif head:
        head_id = get_branch_id(head)
        if not head_id:
            return "the given head does not exist in the current working directory"
        head_path = os.path.join(current_dir, '.wit', 'images', head_id)
    else:
        head_id = get_head_id(current_dir)
        head_path = os.path.join(current_dir, '.wit', 'images', head_id)
    
    relative_path = find_relative_path(path, head_path)

    if os.path.isfile(path):
        try:
            with open(path, 'r') as current_file:
                main_file = current_file.readlines()
            with open(relative_path, 'r') as backup_file:
                backup_file = backup_file.readlines()
        
        except UnicodeDecodeError:
            return f"binary file at {file_name}"

        else:
            diff = difflib.unified_diff(main_file, backup_file)
            return '\n'.join(diff)
    else:
        return return_value(path, relative_path)


def compare_commits(commit1, commit2):
    current_dir = os.getcwd()
    
    file1 = os.path.join(current_dir, '.wit', 'images', commit1)
    file2 = os.path.join(current_dir, '.wit', 'images', commit2)

    if not os.path.exists(file1) or not os.path.exists(file2):
        return "sorry, couldn't find your commits"

    return return_value(file1, file2)


def compare_branches(branch1, branch2):
    current_dir = os.getcwd()
    branch_id1 = get_branch_id(branch1)
    branch_id2 = get_branch_id(branch2)

    file1 = os.path.join(current_dir, '.wit', 'images', branch_id1)
    file2 = os.path.join(current_dir, '.wit', 'images', branch_id2)

    return return_value(file1, file2)


def branch_compare_file(branch1, branch2, file):
    if not check_if_branch(branch1) or not check_if_branch(branch2):
        return "sorry, couldn't find your branches"

    current_dir = os.getcwd()
    branch_id1 = get_branch_id(branch1)
    branch_id2 = get_branch_id(branch2)

    file1 = os.path.join(current_dir, '.wit', 'images', branch_id1, file)
    file2 = os.path.join(current_dir, '.wit', 'images', branch_id2, file)

    if not os.path.exists(file1) or not os.path.exists(file2):
        return "sorry, couldn't find your file in your branches"
    
    if os.path.isfile(file1) and os.path.exists(file2):
        try:
            with open(file1, 'r') as current_file:
                file1_data = current_file.readlines()
            with open(file2, 'r') as backup_file:
                file2_data = backup_file.readlines()

        except UnicodeDecodeError:
            return f"binary file at {file}"

        else:
            diff = difflib.unified_diff(file1_data, file2_data)
            return '\n'.join(diff)
    else:
        return return_value(file1, file2)
...
def diff(*sources):
    if not sources:
        ...
        if not os.path.isabs(sources[0]):
            print("single_file func takes a full path to a file")
        else:
            print(single_file(*sources))
    elif len(sources) == 2:
        if os.path.exists(sources[1]) and os.path.isabs(sources[1]):
            print(single_file(sources[1], sources[0]))
        else:
            if check_if_branch(sources[0]) and check_if_branch(sources[1]):
                print(compare_branches(*sources))
            else:
                print(compare_commits(*sources))
    elif len(sources) == 3:
        print(branch_compare_file(*sources))
    else:
